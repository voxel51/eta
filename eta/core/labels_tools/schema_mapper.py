...
from .schema_filters import SchemaFilter, VideoAttrFilter, FrameAttrFilter, \
    ImageAttrFilter, DetectedObjectFilter, DetectedObjectAttrFilter, \
...
class SchemaMapper(etas.Serializable):
    ...
    _VALID_CLASS_MAPS = {
        ...
        DetectedObjectFilter: [DetectedObjectAttrFilter],
        EventFilter: [EventAttrFilter],
        DetectedObjectAttrFilter: [DetectedObjectFilter],
    ...
    def map_labels(self, labels: Union[ImageLabels, VideoLabels]):
        ...
        elif isinstance(self.filter, DetectedObjectFilter):
            for obj in self.filter.iter_matches(labels):
                self._process_object(obj)
        elif isinstance(self.filter, DetectedObjectAttrFilter):
    ...
    def _process_object(self, obj: etao.DetectedObject):
        ...
        if isinstance(self.output_map, DetectedObjectAttrFilter):
    ...
    def _process_object_attr(
        ...
        if isinstance(self.output_map, DetectedObjectFilter):
