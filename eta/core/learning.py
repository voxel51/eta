...
class ExposesClassificationProbabilities(object):
    '''Mixin for `Classifier` subclasses that expose probabilities for their
    predictions.
    '''

    @property
    def exposes_probabilities(self):
        '''Whether this classifier exposes probabilities for its predictions.

        This property allows for the possibility that some, but not all
        instances of a `Classifier` are capable of exposing probabilities.
        '''
        raise NotImplementedError(
    ...
    def class_labels(self):
        '''The list of class labels for the classifier.'''
    ...
    def get_probabilities(self):
        ...
        Returns:
            a vector of class probabilities, or None if the classifier has not
    ...
    @classmethod
    def ensure_exposes_probabilities(cls, classifier):
        '''Ensures that the given classifier exposes probabilities.

        Args:
            classifier: a Classifier

        Raises:
            ValueError: if `classifier` does not expose probabilities
        '''
        if not isinstance(classifier, cls):
            raise ValueError(
                "Expected %s to implement the %s mixin, but it does not" %
        ...
        if not classifier.exposes_probabilities:
            raise ValueError(
                "Expected %s to expose probabilities, but it does not" %
                type(classifier))


class ExposesDetectionProbabilities(object):
    '''Mixin for `Detector` subclasses that expose probabilities for their
    detections.
    '''

    @property
    def exposes_probabilities(self):
        '''Whether this detector exposes probabilities for its detections.

        This property allows for the possibility that some, but not all
        instances of a `Detector` are capable of exposing probabilities.
        '''
        raise NotImplementedError(
            "subclasses must implement exposes_probabilities")
    ...
    @property
    def class_labels(self):
        '''The list of class labels for the detector.'''
        raise NotImplementedError("subclasses must implement class_labels")
    ...
    def get_probabilities(self):
        ...
        Returns:
            a list of class probability vectors, or None if the detector has
                not (or does not) generated probabilities
        '''
        raise NotImplementedError(
            "subclasses must implement get_probabilities()")

    @classmethod
    def ensure_exposes_probabilities(cls, detector):
        '''Ensures that the given detector exposes probabilities.

        Args:
            detector: a Detector

        Raises:
            ValueError: if `detector` does not expose probabilities
        '''
        if not isinstance(detector, cls):
            raise ValueError(
                "Expected %s to implement the %s mixin, but it does not" %
        ...
        if not detector.exposes_probabilities:
            raise ValueError(
                "Expected %s to expose probabilities, but it does not" %
                type(detector))
