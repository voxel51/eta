...
bounding boxes, polylines, polygons, etc.
...
class Polyline(Serializable):
    ...
    def __init__(
        self, name=None, label=None, points=None, closed=False, filled=False
    ):
        ...
        Args:
            ...
            points (None): a list of (x, y) points in [0, 1] x [0, 1]
                describing the vertexes of a curve
    ...
    def coords_in(self, frame_size=None, shape=None, img=None):
        """Returns the coordinates of the polyline vertexes in the specified
        image.
        ...
        Returns:
            a list of (x, y) vertexes in pixels
        """
        w, h = _to_frame_size(frame_size=frame_size, shape=shape, img=img)
    ...
    def from_abs_coords(
        ...
        """Constructs a Polyline from absolute pixel coordinates.

        One of `frame_size`, `shape`, or `img` must be provided.

        Args:
            points: a list of (x, y) keypoints in [0, 1] x [0, 1] describing
                the vertexes of the polyline
            name (None): a name for the polyline, e.g., ``ground_truth`` or the
                name of the model that produced it
            label (None): a label for the shape
            closed (False): whether the polyline is closed, i.e., an edge
            ...
            filled (False): whether the polyline represents a shape that can
                be filled when rendering it
            clamp: whether to clamp the relative points to [0, 1] x [0, 1], if
                necessary. By default, this is True
            frame_size: the (width, height) of the image
            shape: the (height, width, ...) of the image, e.g. from img.shape
            img: the image itself
    ...
    @classmethod
    def from_dict(cls, d):
        """Constructs a Polyline from a JSON dictionary.
        ...
        return cls(
            name=name, label=label, points=points, closed=closed, filled=filled
        )


class PolylineContainer(Container):
    """A ``eta.core.serial.Container`` of Polyline instances."""
...
class Keypoints(Serializable):
    """A collection of keypoints in an image, represented as a list of (x, y)
    coordinates in [0, 1] x [0, 1].
    """

    def __init__(self, points=None):
        """Creates a Keypoints instance.
    ...
    def add(self, keypoints):
        """Adds the Keypoints to this instance.

        Args:
            keypoints: a Keypoints
        """
        self.points.extend(keypoints.points)
    ...
    @classmethod
    def from_abs_coords(
        cls, points, clamp=True, frame_size=None, shape=None, img=None,
    ):
        """Constructs a Keypoints from absolute pixel coordinates.
        ...
        return cls(points=rpoints)

    @classmethod
    def from_dict(cls, d):
        """Constructs a Keypoints from a JSON dictionary.
