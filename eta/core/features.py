...
class VideoFeaturizer(Featurizer):
    ...
    **WARNING** if you use the same backing path for multiple videos your
    ...
    def __init__(self, config):
        ...
        super(VideoFeaturizer, self).__init__()
    ...
    def featurize(self, video_path, returnX=True):
        ...
        self.start(warn_on_restart=False, keep_alive=False)
        v = self._featurize(video_path, returnX)
        if self._keep_alive is False:
            self.stop()
    ...
    def _featurize(self, video_path, returnX=True):
        if returnX:
            X = None
        ...
        if self._frame_preprocessor is not None:
            ...
            v = self._frame_featurizer.featurize(video_path)
        else:
            # Featurize the frame
            v = self._frame_featurizer.featurize(video_path)
