...
class ClassifyFramesConfig(etam.BaseModuleConfig):
    ...
    def __init__(self, d):
        super(ClassifyFramesConfig, self).__init__(d)
...
def _process_video(data, classifier, parameters):
    ...
    with etav.FFmpegVideoReader(data.video_path) as vr:
        for img in vr:
            ...
            labels.add_frame_attributes(attrs, vr.frame_number)
...
def run(config_path, pipeline_config_path=None):
    '''Run the classify_frames module.

    Args:
        config_path: path to a ClassifyFramesConfig file
        pipeline_config_path: optional path to a PipelineConfig file
    '''
    config = ClassifyFramesConfig.from_json(config_path)
    etam.setup(config, pipeline_config_path=pipeline_config_path)
    _classify_frames(config)
